...
function Tini() {
  ...
  addEventListener("fetch", event =>
    handle(event.request).then(data => event.respondWith(data))
  );
  ...
  return {
    get: (route, ...cbs) => {
      return response("GET", route, cbs);
    },
    post: (route, ...cbs) => {
      return response("POST", route, cbs);
    },
    put: (route, ...cbs) => {
      return response("PUT", route, cbs);
    },
    del: (route, ...cbs) => {
      return response("DELETE", route, cbs);
    }
  };
}
return new Tini();
